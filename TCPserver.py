...
def KOHA():
    return str(datetime.now().strftime("%d.%m.%Y %H:%M:%S %p"))
...
def threadi_klient(connectionSocket,address):
    try:
        kerkesa = connectionSocket.recv(128)                                                          #kerkesa me e madhe qe mund te pranoj serveri eshte 128 bajt
        kerkesa_e_dekoduar = str(kerkesa.decode('utf-8'))
    except socket.error as e:
        print(str(e))
        print("Klienti mbylli lidhjen")
        connectionSocket.close()
        return
    if kerkesa_e_dekoduar == 'IP':
        pergjiegjja = IP(address);
        connectionSocket.sendall(str.encode(pergjiegjja))                                                 #me provu a po gjun exception 
        connectionSocket.close()
    elif kerkesa_e_dekoduar == 'NRPORTIT':
        pergjiegjja = NRPORTIT(address);
        connectionSocket.sendall(str.encode(pergjiegjja))
        connectionSocket.close()
        ...
        connectionSocket.sendall(str.encode("Kerkesa nuk ishte valide!!!"))
        connectionSocket.close()
...
while True:
    ...
    print('Serveri u lidh me klientin me IP Adrese '+address[0]+" i cili eshte duke perdorur portin "+str(address[1]))
    print('-----------------------------------------------------------')
    start_new_thread(threadi_klient,(clientS,address,)) 
    
    
#ky funksione fillon nje thread te ri, konkretisht e ekzekuton funksionin threadi_klient si thread duke ia derguar si parameter soketin clientS
